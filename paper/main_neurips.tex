...
% For preprint with author names: \usepackage{neurips_2026}  % Anonymous NeurIPS submission
\usepackage{neurips_2026}  % Anonymous NeurIPS submission  % arXiv preprint
...
(3{,}072 $-$ 27 named $=$ 3{,}045; we round for readability).
...
residual stream from attention, with success scaling with how constrained
the context is.
A garden-path experiment reveals that GPT-2
uses verb subcategorization immediately, with a suggestive but non-significant
reversal of the classical garden-path effect, consistent with the exception handler
...
The consensus-crossover (\S\ref{sec:consensus}) confirms that the handler
operates at token-level predictability, not syntactic structure;
a garden-path experiment provides suggestive but non-significant
corroborating evidence (\S\ref{sec:garden_path}, Appendix~\ref{app:garden_path}).
...
\begin{verbatim}
 ...
 RESIDUAL(3,072 - 27 = 3,045 neurons):
...
\paragraph{Data.}
All experiments use 512{,}000 tokens from WikiText-103, processed in the
first 500 contiguous sequences of 1{,}024 tokens from the validation set.
...
in the enrichment analysis (Table~\ref{tab:enrichment_stats}).
...
\subsection{Seven Monitors of Normal}
...
\footnote{In \emph{Paradise Lost}, Moloch counsels
perpetual war regardless of outcome---``My sentence is for open war''---preferring
action to deliberation. The MLP shares this compulsion: it intervenes on every
token, even when intervention is counterproductive.}
...
reveals a suggestive but non-significant trend: GPT-2 uses verb
subcategorization immediately (``struggled'' cannot take an object), so
the intransitive condition produces lower surprisal at disambiguation
in 8 of 15 pairs ($W = 52$, $p = 0.65$, median $\Delta = +0.3$ bits).
...
This parallels human sentence processing: \citet{trueswell1993verb} demonstrated
that readers use verb subcategorization information immediately, with
garden-path effects occurring only for NP-bias verbs (e.g., \emph{forgot})
but not for S-bias verbs (e.g., \emph{insisted})---the same asymmetry GPT-2
exhibits.
...
Of the 20 prompts, 17 showed increased target probability when Dai's neurons
were zeroed (range: $+$0.3pp to $+$24.3pp), 2 showed negligible change
($< \pm$0.1pp), and 1 showed a decrease ($-$2.1pp).
The consistency across diverse facts (85\% showing improvement) confirms
that these neurons systematically suppress factual signal rather than
...
(204{,}800 tokens). ``First lock-in (Tuned)'' = percentage of tokens that
first reach top-1 accuracy at this layer under the tuned lens.}
...
Following \citet{trueswell1993verb} and \citet{vangompel2001lexical}, we constructed 15 minimal pairs
...
\midrule
\multicolumn{4}{@{}l}{Median $\Delta$ (trans $-$ int)} & \multicolumn{2}{c}{$+$0.3 bits} \\
\bottomrule
\end{tabular}
\end{table}
...
powered for $d \geq 0.78$).
The transitive condition produces higher surprisal at disambiguation
in 8 of 15 pairs, but this trend is not statistically significant
(Wilcoxon $W = 52$, $p = 0.65$, median $\Delta = +0.3$ bits).
The two strongest pairs (``scratched'' $+$9.4, ``struggled'' $+$6.2)
drive the positive mean ($+$0.8 bits), but the effect is inconsistent
across the set.
...
Where subcategorization is unambiguous, the garden-path is
\emph{reversed}: transitive verbs create the reparse.
However, the small and variable effect size (8/15 pairs, $p = 0.65$)
means this reversal is hypothesis-generating rather than confirmatory,
consistent with our framing in \S\ref{sec:intro}.

\paragraph{Connection to constraint-based parsing.}
\citet{trueswell1993verb} separated lexical preference effects from true
garden-paths, showing that verb subcategorization is used immediately in human
parsing---contra lexical-filtering models where subcategorization is only
consulted after initial structure-building
\citep[see also][for related evidence on lexical mediation]{traxler2007lexical}.
GPT-2's N2123 null result is consistent with this picture: verb-specific
constraints appear to be applied at the verb itself rather than deferred to
a later reanalysis stage, though the non-significant surprisal effect
($p = 0.65$) means this remains a hypothesis rather than a confirmed finding.
...
immediately without exception-handler involvement, paralleling both Trueswell
et al.'s constraint-based account and Britt et al.'s autonomous syntactic
component.
...
\item \textbf{SAE alternative}: We characterize neurons individually rather than
via sparse autoencoders \citep{bricken2023monosemanticity, templeton2024scaling},
which might resolve additional structure within the $\sim$3{,}045 residual
neurons our manual method classifies as entangled.
\end{itemize}

\section{Broader Impact}
\label{sec:broader_impact}

This work contributes to mechanistic interpretability by providing a replicable template for decomposing MLP layers into functionally distinct neuron groups. The exception-handler framing may improve safety monitoring: if harmful content generation is mediated by specific neurons' routing decisions, the consensus/exception decomposition could help localize and audit those pathways. Our finding that model editing (ROME) modifies routing rather than stored knowledge has implications for alignment: interventions targeting ``knowledge editing'' may have different behavioral consequences than intended. We note the usual dual-use concern that better mechanistic understanding could also facilitate adversarial manipulation, though we believe the interpretability benefits outweigh this risk at GPT-2 Small's scale.
