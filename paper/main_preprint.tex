...
residual stream from attention, with capability indexed by contextual
constraint.
...
autoencoders; \citet{templeton2024scaling} extended this to production models,
suggesting SAE decomposition could resolve further structure within our
$\sim$3{,}040 residual neurons.
...
At Layer~11, the MLP output is decomposed:
$\text{MLP}(x) = \text{Core}(x) + \text{Diff}(x) + \text{Spec}(x) + \text{Residual}(x)$,
where each tier applies a binary mask to post-GELU activations before projecting
through the output matrix.
The sum equals the original exactly
...
At the generic threshold, N2123 activates on 70.9\% of tokens.
However, only 11.3\% produce the \emph{high-magnitude} activations that
co-occur with exception-path behavior.
...
500 sequences) to account for within-sequence autocorrelation.
...
This complements \citet{geva2023dissecting}, who found mid-layer MLPs
\emph{promote} correct attributes.
We believe the findings are complementary: mid-layer MLPs receive
partially formed predictions; the terminal MLP's role shifts from promotion
to routing.
...
(Wilcoxon $W = 12$, $p = 0.018$, median $+$3.1 bits).
